...
def uppercase_regions(seq_record):
    start = None
    for i, nucleotide in enumerate(seq_record.seq):
        if nucleotide.isupper() and start is None:
            start = i
        elif nucleotide.islower() and start is not None:
            yield (start, i)
            start = None
    ...
    with open(output_file, 'w') as out:
        ...
        with open_func(input_file, 'rt') as handle:
            for record in SeqIO.parse(handle, 'fasta'):
                for start, end in uppercase_regions(record):
                    out.write(f"{record.id}\t{start}\t{end}\n")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: uppercase_to_bed.py <softmasked_fasta(.gz> <output_bed>")
        sys.exit(1)
